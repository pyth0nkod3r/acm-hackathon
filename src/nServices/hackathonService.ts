  ...
  async submitRegistration(
    ...
    try {
      ...
      formData.teamMembers.forEach((member, index) => {
        form.append(
          `teamMembers[${index}][teamMemberFullName]`,
          member.teamMemberFullName
        );
        form.append(
          `teamMembers[${index}][teamMemberEmail]`,
          member.teamMemberEmail
        );
        form.append(
          `teamMembers[${index}][teamMemberPhone]`,
          member.teamMemberPhone
        );
        form.append(
          `teamMembers[${index}][teamMemberRole]`,
          member.teamMemberRole
        );
        form.append(
          `teamMembers[${index}][teamMemberLinkedIn]`,
      ...
      const response =
        await apiService.publicPost<HackathonRegistrationResponse>(
          '/api/hackathon',
          form
        );
